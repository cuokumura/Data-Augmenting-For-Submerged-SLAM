...
void Tracking::GrabDepthData(const DEPTH::Point &depthMeasurement)
{
    unique_lock<mutex> lock(mMutexDepthQueue);
    mlQueueDepthData.push_back(depthMeasurement);
}
...
void Tracking::PrepareDepth()
{
      ...
      while(true)
    {
        bool bSleep = false;
        {
            ...
            {
                DEPTH::Point* m = &mlQueueDepthData.front();
                cout.precision(17);
                ...
                {
                    mvDepthFromLastFrame.push_back(*m);
                    mlQueueDepthData.pop_front();
                }
                else
                {
...
{
//    std::cout << "Trying potredict pose IMU: ";
    if(!mCurrentFrame.mpPrevFrame)
    {
    ...
    //    std::cout << "Code 0" << std::endl;
        ...
        if (mvDepthFromLastFrame.size() > 0)
        {
            twb2.z() = -mvDepthFromLastFrame.begin()->d;
            cout << "1Setting z-value to: " << mvDepthFromLastFrame.begin()->d << std::endl;
        }
    ...
    //    std::cout << "Code 1" << std::endl;
        ...
        {
            twb2.z() = -mvDepthFromLastFrame.begin()->d;
            cout << "2Setting z-value to: " << mvDepthFromLastFrame.begin()->d << std::endl;
        }
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            {
                std::cout << "mState not OK" << std::endl;
...
void Tracking::UpdateFrameIMU(const float s, const IMU::Bias &b, KeyFrame* pCurrentKeyFrame)
{
    ...
    {
        const Eigen::Vector3f Gz(0, 0, -IMU::GRAVITY_VALUE);
        Eigen::Vector3f twb1 = mLastFrame.mpLastKeyFrame->GetImuPosition();
        ...
        if (mvDepthFromLastFrame.size() > 0)
        {
            twb1.z() = -mvDepthFromLastFrame.begin()->d;
        }
    ...
    if (mCurrentFrame.mpImuPreintegrated)
    {
        ...
        Eigen::Vector3f twb1 = mCurrentFrame.mpLastKeyFrame->GetImuPosition();
