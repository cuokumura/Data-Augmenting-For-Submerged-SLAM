...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor, const depthSensor depthSensor,
               const bool bUseViewer, const int initFr, const string &strSequence):
    mSensor(sensor), mDepthSensor(depthSensor), mpViewer(static_cast<Viewer*>(NULL)), mbReset(false), mbResetActiveMap(false),
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    cout << "Using depth sensor is set to: ";
    if (mDepthSensor == DEPTH_DISABLED)
        cout << "Disabled" << endl;
    else if (mDepthSensor == DEPTH_ENABLED)
        cout << "Enabled" << endl;
...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, const vector<DEPTH::Point>& vDepthMeas, string filename)
{
    ...
    if (mDepthSensor == System::DEPTH_ENABLED)
        for (size_t i_depth=0; i_depth < vDepthMeas.size(); i_depth++)
            mpTracker->GrabDepthData(vDepthMeas[i_depth]);
